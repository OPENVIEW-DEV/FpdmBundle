<?php
// <NICOLA> Namespace aggiunti da Nicola
namespace Openview\FpdmBundle\Model;

use Openview\FpdmBundle\Utils\FilterStandard;
use Openview\FpdmBundle\Utils\FilterASCII85;
use Openview\FpdmBundle\Utils\FilterASCIIHex;
use Openview\FpdmBundle\Utils\FilterFlate;
use Openview\FpdmBundle\Utils\FilterLZW;
...
if (!call_user_func_array('class_exists', $__tmp)) {
    ...
    class FPDM {
        // <NICOLA> trasformato in proprietà della classe
        var $FPDM_FILTERS = array(
            "ASCII85Decode",
            "Standard",
            "FilterLZW",
            "FilterFlate",
        ...
        function _set_field_value($line, $value) {
            ...
            // <NICOLA> Modifica richiesta dall'adattamento
            $callback_code = '$THIS=new Openview\FpdmBundle\Model\FPDM("[_STATIC_]");return $THIS->replace_value($matches,"' . $value . '");';
            ...
            if (isset($this->value_entries["$name"])) {
                ...
                if ($type == "tooltip") {
                    ...
                    if (isset($this->value_entries["$name"]["values"]["$type"])) {
                        ...
                        if ($verbose_set)
                            echo "<br>Change $type value of the field $name at line $field_value_line to '<i>$value</i>'";
                        ...
                        if ($obj) {
                            ...
                            if (preg_match("/endobj/", $CurLine, $match)) {
                                ...
                                if (($type == 'Annot') && ($subtype == "Widget")) { 
                                    ...
                                    // @todo <ASINELLO> Rimettere il controllo quando sistemato
                                    if (false) {
                                        if (!array_key_exists("current", $values)) {
                                            dump($values);
                                            $this->Error("Cannot find current Value (/v)  for annotation object $obj,<br>Something goes wrong during parsing (uncompress your pdf and retry) or your pdf is corrupted.");
                                        }
                                        if (!array_key_exists("default", $values))
                                            $this->Error("Cannot find default Value (/DV) for annotation object $obj,<br>Something goes wrong during parsing (uncompress your pdf and retry) or your pdf is corrupted.");
                                    }
                                }else
                                if ($verbose_parsing)
